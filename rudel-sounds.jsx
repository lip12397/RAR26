// rudel-sounds.jsx — Web-Audio-Engine mit witzigen, variierenden Soundeffekten.
// Jeder "Moment" hat mehrere Varianten, die zufällig durchrotieren. Keine Audio-Dateien.

const RUDEL_SOUND_KEY = 'rudel_v1_muted';
...
const RudelSound = (function () {
  ...
  function setMuted(v) {
    muted = !!v;
    try { localStorage.setItem(RUDEL_SOUND_KEY, muted ? 'true' : 'false'); } catch (e) {}
...
function useRudelMuted() {
  const [m, setM] = React.useState(RudelSound.isMuted());
  React.useEffect(() => RudelSound.subscribe(setM), []);
  return m;
}

function RudelSoundToggle({ style = {} }) {
  const muted = useRudelMuted();
  return (
    <button
      onClick={() => RudelSound.toggle()}
      aria-label={muted ? 'Sound einschalten' : 'Sound ausschalten'}
      style={{
        background: 'transparent',
        border: `1.5px solid ${muted ? 'rgba(246,245,241,0.25)' : PALETTE.B}`,
        borderRadius: 10, width: 38, height: 38, padding: 0, cursor: 'pointer',
        display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
        color: muted ? PALETTE.dim : PALETTE.B, flexShrink: 0,
        ...style,
      }}>
      {muted ? (
...
Object.assign(window, { RudelSound, useRudelMuted, RudelSoundToggle });
